...
@section('title', 'Protocol Type')

@section('content_header')
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1 class="m-0"><i class="fas fa-network-wired mr-2 text-warning"></i>Protocol Type</h1>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{ url('dashboard') }}">Home</a></li>
                <li class="breadcrumb-item">Settings</li>
                <li class="breadcrumb-item active">Protocol Type</li>
            </ol>
        </div>
    </div>
@endsection
...
<div class="container-fluid">
            ...
            <div class="row align-items-end">
                <div class="col-md-5">
                    ...
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" id="filter-search" class="form-control" placeholder="Protocol type name...">
                    </div>
                </div>
                <div class="col-md-3">
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h3 class="card-title"><i class="fas fa-list mr-1"></i> Protocol Type List</h3>
            <div>
                <button class="btn btn-warning btn-sm" id="btnAdd">
                    <i class="fas fa-plus mr-1"></i> Protocol Type
                </button>
                <span class="badge badge-info ml-2" id="badge-count">0</span>
            </div>
        </div>
        <div class="card-body p-0">
            <table id="ptTable" class="table table-bordered table-striped table-hover table-sm mb-0">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-warning">
                <h5 class="modal-title"><i class="fas fa-plus-circle mr-2"></i>Add Protocol Type</h5>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <div class="modal-body">
                <div class="form-group">
                    <label>Protocol Type Name <span class="text-danger">*</span></label>
                    <input type="text" id="add_name" class="form-control" placeholder="e.g. PPPoE, DHCP, Static...">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-info">
                <h5 class="modal-title text-white"><i class="fas fa-edit mr-2"></i>Edit Protocol Type</h5>
...
<style>
    ...
    #ptTable thead th { font-size: 13px; }
</style>
@endsection
...
$(function () {
    ...
    var table = $('#ptTable').DataTable({
        processing: true,
        ajax: {
            url: '{{ route("settings.protocol-types.data") }}',
        ...
        columns: [
            ...
            { data: 'name' },
            { data: 'details', render: v => v ? v : '<span class="text-muted">—</span>' },
            {
                ...
                render: function(val, type, row) {
                    ...
                    return `<span class="toggle-switch btn-toggle" data-id="${row.id}">${icon}</span>`;
                }
            },
            {
                ...
                render: function(id, type, row) {
                    return `
                        <button class="btn btn-xs btn-success btn-edit mr-1"
                            ...
                            data-details="${row.details || ''}" data-active="${row.is_active}">
        ...
        pageLength: 10,
        language: {
            emptyTable: 'No protocol types found.',
            zeroRecords: 'No protocol types found.',
    ...
    $('#btnReset').on('click', function() {
        $('#filter-search').val(''); $('#filter-status').val('');
        $.fn.dataTable.ext.search = []; table.draw();
    });

    // ── Add ────────────────────────────────────────────────
    $('#btnAdd').on('click', function() {
        $('#add_name').val('').removeClass('is-invalid');
        $('#add_details').val(''); $('#add_name_error').text('');
        $('#addModal').modal('show');
    });
    ...
    $('#btnSave').on('click', function() {
        ...
        $.ajax({
            url: '{{ route("settings.protocol-types.store") }}',
            method: 'POST',
            data: { _token: '{{ csrf_token() }}', name: $('#add_name').val(), details: $('#add_details').val() },
            success: function(res) {
                if (res.success) { $('#addModal').modal('hide'); table.ajax.reload(null, false); toastr.success(res.message); }
            },
            error: function(xhr) {
            ...
            complete: function() { $('#btnSave').prop('disabled', false).html('<i class="fas fa-save mr-1"></i>Save'); }
        });
    });
    ...
    $(document).on('click', '.btn-edit', function() {
        var d = $(this).data();
        $('#edit_id').val(d.id); $('#edit_name').val(d.name).removeClass('is-invalid');
        $('#edit_details').val(d.details); $('#edit_active').val(d.active == 1 ? '1' : '0');
        $('#edit_name_error').text(''); $('#editModal').modal('show');
    });
    ...
    $('#btnUpdate').on('click', function() {
        ...
        $.ajax({
            url: '/settings/protocol-types/' + $('#edit_id').val(),
            method: 'PUT',
            data: { _token: '{{ csrf_token() }}', name: $('#edit_name').val(), details: $('#edit_details').val(), is_active: $('#edit_active').val() },
            success: function(res) {
                if (res.success) { $('#editModal').modal('hide'); table.ajax.reload(null, false); toastr.success(res.message); }
            },
            error: function(xhr) {
                var errors = xhr.responseJSON?.errors || {};
            ...
            complete: function() { $('#btnUpdate').prop('disabled', false).html('<i class="fas fa-save mr-1"></i>Update'); }
        });
    });
    ...
    $(document).on('click', '.btn-toggle', function() {
        var id = $(this).data('id');
        $.ajax({
            url: '/settings/protocol-types/' + id + '/toggle',
            method: 'POST', data: { _token: '{{ csrf_token() }}' },
            success: function(res) { if (res.success) { table.ajax.reload(null, false); toastr.info(res.message); } },
    ...
    $(document).on('click', '.btn-delete', function() {
        var id = $(this).data('id');
        Swal.fire({
            title: 'Delete Protocol Type?', text: 'This cannot be undone.', icon: 'warning',
            showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#6c757d',
            confirmButtonText: 'Yes, Delete!', cancelButtonText: 'Cancel',
        }).then(function(result) {
            if (result.isConfirmed) {
                $.ajax({
                    url: '/settings/protocol-types/' + id,
                    method: 'DELETE', data: { _token: '{{ csrf_token() }}' },
                    success: function(res) { table.ajax.reload(null, false); toastr.success(res.message); },
                    error: function(xhr) { toastr.error(xhr.responseJSON?.message || 'Failed to delete.'); }
                });
            }
        });
    });
