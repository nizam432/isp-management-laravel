                    ...
                    <div class="input-group">
                        <select name="connection_type_id" id="connectionTypeSelect" class="form-control">
                            ...
                            @foreach($connectionTypes as $ct)
                                <option value="{{ $ct->id }}" {{ old('connection_type_id') == $ct->id ? 'selected' : '' }}>
                                    {{ $ct->name }}
                                </option>
                            @endforeach
                        </select>
                        <div class="input-group-append">
                            <button type="button" class="btn btn-outline-success"
                                    data-toggle="modal" data-target="#modalConnectionType" title="Add Connection Type">
                                <i class="fas fa-plus"></i>
                            </button>
                        </div>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="input-group">
                        <input type="number" name="monthly_bill_amount" id="billAmount"
                               ...
                               placeholder="Auto from package" step="0.01" min="0">
                        <div class="input-group-append">
                            <span class="input-group-text">BDT</span>
                        </div>
                    </div>
                    <small class="text-muted">Leave empty = package price</small>
                </div>
            </div>
            <div class="col-md-3">
                <div class="form-group">
                    ...
                    <div class="input-group">
                        <select name="protocol_type_id" id="protocolTypeSelect" class="form-control">
                            ...
                            @foreach($protocolTypes as $pt)
                                <option value="{{ $pt->id }}" {{ old('protocol_type_id') == $pt->id ? 'selected' : '' }}>
                                    {{ $pt->name }}
                                </option>
                            @endforeach
                        </select>
                        <div class="input-group-append">
                            <button type="button" class="btn btn-outline-success"
                                    data-toggle="modal" data-target="#modalProtocolType" title="Add Protocol Type">
                                <i class="fas fa-plus"></i>
                            </button>
                        </div>
                    </div>
...
$('#zoneSelect').on('change', function() {
    ...
    $.get('/customers/sub-zones', { zone_id: zoneId }, function(data) {
        ...
        data.forEach(function(sz) { opts += '<option value="' + sz.id + '">' + sz.name + '</option>'; });
        $sub.html(opts);
