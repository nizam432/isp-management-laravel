    ...
    <div class="d-flex justify-content-between align-items-center">
        <div>
            <h4 class="mb-0 font-weight-bold text-dark">
                <i class="fas fa-undo-alt mr-2 text-primary"></i>{{ $return->return_no }}
            </h4>
            <small class="text-muted">Sale return details</small>
        </div>
        ...
        <div class="card shadow-sm mb-3">
            ...
            <div class="card-body p-0">
                <table class="table table-sm mb-0">
                    <tr><td class="small text-muted pl-3" style="width:45%">Sale Invoice</td>
                        <td class="pr-3"><a href="{{ route('inventory.sales.show', $return->sale_id) }}" class="font-weight-bold">{{ $return->sale->invoice_no ?? '—' }}</a></td></tr>
                    <tr><td class="small text-muted pl-3">Customer</td>
                        <td class="pr-3">{{ $return->client->name ?? $return->sale->walk_in_name ?? 'Walk-in' }}</td></tr>
                    ...
                    <tr><td class="small text-muted pl-3">Refund Type</td>
                        <td class="pr-3"><span class="badge badge-light border">{{ ucfirst($return->refund_type) }}</span></td></tr>
