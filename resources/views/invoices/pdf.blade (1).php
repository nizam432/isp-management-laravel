...
<div class="header">
    <div class="header-inner">
        <div class="header-left">
            <?php if($logoBase64): ?>
                <img src="{{ $logoBase64 }}" style="height:40px; margin-bottom:6px;"><br>
            <?php endif; ?>
...
@php
    ...
    $prevDue     = $invoice->previous_due ?? 0;

    $numberToWords = function(float $number): string {
        ...
        if ($n === 0) return 'Zero';
        $result = '';
        if ($n >= 1000) { $result .= $ones[(int)($n/1000)] . ' Thousand '; $n %= 1000; }
        if ($n >= 100)  { $result .= $ones[(int)($n/100)]  . ' Hundred ';  $n %= 100; }
        if ($n >= 20)   { $result .= $tens[(int)($n/10)]   . ' ';          $n %= 10; }
        if ($n > 0)     { $result .= $ones[$n] . ' '; }
        return trim($result) . ' Taka Only';
    };

    $qrUrl = url('/client/pay/' . $invoice->id . '/select?pay_all=1');

    // endroid/qr-code — GD writer, no Imagick needed
    $qrCode = \Endroid\QrCode\QrCode::create($qrUrl)
        ->setSize(90)
        ->setMargin(4);
...
<table style="width:100%; border-collapse:collapse; border-top:1px solid #eee; border-bottom:1px solid #eee;">
    <tr>
        <td style="width:120px; padding:12px 16px; text-align:center; vertical-align:middle; border-right:1px solid #eee;">
            <div style="font-size:10px; color:#00a65a; font-weight:bold; text-transform:uppercase; letter-spacing:0.5px; margin-bottom:5px;">Scan to Pay</div>
            <img src="{{ $qrBase64 }}" width="90" height="90" style="display:block; margin:0 auto;">
            <div style="font-size:10px; color:#aaa; margin-top:4px;">{{ $invoice->invoice_no }}</div>
        </td>
        <td style="padding:12px 20px; vertical-align:middle;">
            <table style="width:100%; font-size:13px; border-collapse:collapse;">
                <tr>
                    <td style="padding:3px 0; color:#777;">In Total:</td>
                    <td style="text-align:right; font-weight:bold;">{{ $currency }} {{ number_format($invoice->amount, 2) }}</td>
                </tr>
                <tr>
                    <td style="padding:3px 0; color:#777;">Advance:</td>
                    <td style="text-align:right;">{{ $currency }} {{ number_format($advancePaid, 2) }}</td>
                </tr>
                <tr>
                    <td style="padding:3px 0; color:#777;">Prev Due:</td>
                    <td style="text-align:right;">{{ $currency }} {{ number_format($prevDue, 2) }}</td>
                </tr>
                <tr><td colspan="2" style="padding:4px 0;"><div style="border-top:2px solid #00a65a;"></div></td></tr>
                <tr>
                    <td style="padding:4px 0; font-weight:bold; font-size:14px;">Total Due:</td>
                    <td style="text-align:right; font-weight:bold; font-size:16px; color:#00a65a;">{{ $currency }} {{ number_format($invoice->due_amount, 2) }}</td>
                </tr>
                <tr>
                    <td colspan="2" style="padding:3px 0; font-size:12px; color:#555;">
                        <strong>In Words:</strong> {{ $numberToWords($invoice->due_amount) }}
                    </td>
                </tr>
            </table>
        </td>
...
<div class="payment-wrap" style="padding-top:12px;">
    <div class="section-label" style="padding:0 0 6px;">Payment History</div>
...
@if($invoice->notes)
<div style="padding:8px 28px; font-size:12px; color:#666;">
