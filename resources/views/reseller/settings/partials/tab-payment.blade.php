{{--
    resources/views/reseller/settings/partials/tab-payment.blade.php
    Included from reseller/settings/general.blade.php's #tab-payment div.
--}}
@php
    $gwMeta = [
        // ── Local ────────────────────────────────────────────────
...
<div class="row">
    ...
    {{-- ── Left: Gateway List (only ones Super Admin has enabled) ── --}}
    <div class="col-md-4">
        <div class="list-group" id="pgTabList">

            @if($allGateways->where('type', 'local')->isNotEmpty())
            ...
            @foreach($allGateways->where('type', 'local') as $gw)
                @include('reseller.settings.partials._pg-tab-item', ['gw' => $gw, 'gwMeta' => $gwMeta, 'gwSettings' => $gwSettings])
            @endforeach
            @endif

            @if($allGateways->where('type', 'international')->isNotEmpty())
            ...
            @if($allGateways->isEmpty())
                <div class="text-center text-muted py-3">No gateways have been enabled by Admin yet.</div>
            @endif
...
function renderPgConfig(slug) {
    ...
    const saveUrl   = `/reseller/settings/payment-gateways/${slug}/save`;
    const toggleUrl = `/reseller/settings/payment-gateways/${slug}/toggle`;
                ...
                <div class="d-flex justify-content-between align-items-center mt-3">
                    ...
                    <button type="button"
                        ...
                        ${active
                            ...
                            : '<i class="fas fa-toggle-on mr-1"></i> Enable for Clients'}
                    </button>
                </div>
            </div>
        </div>
...
function pgSaveCredentials(slug) {
    ...
    fetch('/reseller/settings/payment-gateways/' + slug + '/save', {
        method: 'POST',
        body: fd,
    })
    .then(res => {
...
function pgToggleGateway(slug, currentlyActive) {
    ...
    fetch(`/reseller/settings/payment-gateways/${slug}/toggle`, {
        method: 'POST',
        body: fd,
    })
    .then(res => {
